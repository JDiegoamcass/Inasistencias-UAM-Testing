...
class ClaseDataSource implements ClaseRepositoryInterface
{
    ...
    public function findAllWithProfesor(): array
    {
        return ClaseEloquent::with('profesor')
            ->get()
            ->map(function($eloquent) {
                return [
                    'clase' => $this->toDomain($eloquent),
                    'profesor' => $eloquent->profesor,
                ];
            })
            ->toArray();
    }
